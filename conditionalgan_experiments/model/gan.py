...
class ConditionalGAN(object):
    def __init__(self, z_dim: int, g_blocks: int, d_blocks: int, out_shape: tuple, num_classes: int, device: torch.device, name: str=None) -> None:
        '''
        Initialize the GAN.
        Parameters:
        ...
        self.generator = Generator(z_dim=self.z_dim, n_blocks=self.g_blocks, out_shape=self.out_shape, num_classes=self.num_classes, name='Generator').to(self.device)

        # Initialize discriminator
        self.discriminator = Discriminator(in_shape=self.out_shape, n_blocks=self.d_blocks, num_classes=self.num_classes, name='Discriminator').to(self.device)
        ...
        for epoch in range(starting_epoch, epochs):
            ...
            with tqdm(dataloader, desc=f'Training : {self.name}') as pbar:
                for imgs, labels in pbar:
                    ...
                    real_labels = Variable(labels.type(torch.LongTensor)).to(self.device)
                    
                    # Recompute batch size for current mini batch
                    batch_size = real_samples.size(0)

                    # Train the discriminator
                    discriminator_loss = self.discriminator_train_loop(k=discriminator_strategy['epochs'], real_samples=real_samples, real_labels=real_labels, batch_size=batch_size, discriminator_optimizer=discriminator_strategy['optimizer'], discriminator_loss_fn=discriminator_strategy['criterion'])
    ...
    def discriminator_train_step(self, real_samples: torch.FloatTensor, real_labels: torch.LongTensor, batch_size: int, discriminator_optimizer: torch.optim, discriminator_loss_fn: torch.nn.Module) -> dict:
        '''
        Each training step of the discriminator.
        Parameters:
            real_samples: The real samples. 
            real_labels: The real labels. 
    ...
    def discriminator_train_loop(self, real_samples: torch.FloatTensor, real_labels: torch.LongTensor, batch_size: int, discriminator_optimizer: torch.optim, discriminator_loss_fn: torch.nn.Module, k: int=1) -> dict:
        '''
        ...
        for _ in range(k):
            ...
            loss = self.discriminator_train_step(batch_size=batch_size, real_samples=real_samples, real_labels=real_labels, discriminator_optimizer=discriminator_optimizer, discriminator_loss_fn=discriminator_loss_fn)
    ...
    def generator_train_step(self, batch_size: int, generator_optimizer: torch.optim, generator_loss_fn: torch.nn.Module) -> float:
        ...
        # Sample labels
        sample_labels = self.sample_labels(batch_size=batch_size)
    ...
    def get_validity_labels(self, batch_size: int, type: str) -> torch.FloatTensor:
        '''
    ...
    def sample_labels(self, batch_size: int) -> torch.LongTensor:
        '''
        Sample labels.
        Parameters:
            batch_size: The batch size.
    ...
    def sample_noise(self, batch_size: int) -> torch.FloatTensor:
        '''
    ...
    def save_batch(self, save_path: str, epoch: int, batch_size: int, loss: int, writer, n_images: int=4) -> None:
        ...
        z = self.sample_noise(batch_size=batch_size)
        # Sample labels
        labels = self.sample_labels(batch_size=batch_size)

        # Set the model to evaluation mode
        self.generator.eval()
        
        # Forward pass to get fake sample
        fake_sample = self.generator(z, labels)
    ...
    def visualize_distribution(self, epoch: int, batch_size: int, dataloader: object, writer: object) -> None:
        ...
        real_samples = Variable(torch.FloatTensor(real_samples)).to(self.device)
        real_labels = Variable(torch.LongTensor(real_labels)).to(self.device)

        writer.add_histogram('Actual Distribution', values=transforms.normalize_tensor(real_samples), global_step=epoch, bins=256)
    ...
    def visualize_model(self, batch_size: int, writer):
        ...
        Returns:
            None
        '''
        generator_samples = self.sample_noise(batch_size=batch_size)
        generator_labels = self.sample_labels(batch_size=batch_size)
        writer.add_graph(self.generator, generator_samples, generator_labels)

        self.generator.eval()
        discriminator_input = self.generator(generator_samples, generator_labels)
        writer.add_graph(self.discriminator, discriminator_input, generator_labels)
