...
class Generator(nn.Module):
    def __init__(self, z_dim: int, n_blocks: int, out_shape: tuple, num_classes: int, name:str=None) -> None:
        '''
        ...
        # Pre input block for embedding
        self.pre_in_block = nn.ModuleDict({
            'embedding_block': nn.Sequential(nn.Embedding(num_embeddings=self.num_classes, embedding_dim=self.num_classes))
        })
        
        # Define input block
        self.in_block = nn.ModuleDict({
            'in_block': nn.Sequential(*block(in_features=self.z_dim + self.num_classes, out_features= 2 * (self.z_dim + self.num_classes), normalize=False, regularize=False))
        })

        # Define intermediate blocks
        self.inter_blocks = nn.ModuleDict({})
        in_dim = 2 * (self.z_dim + self.num_classes)
    ...
    def forward(self, z: torch.FloatTensor, labels: torch.LongTensor) -> torch.FloatTensor:
        '''
