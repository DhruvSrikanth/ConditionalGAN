...
class Experiments():
    ...
    def train(self, verbose: bool=True, checkpoint: dict=None) -> None:
        ...
        # Create the model
        model = ConditionalGAN(z_dim=self.config.hyperparameters['latent dimension'], g_blocks=self.config.generator_config['generator blocks'], d_blocks=self.config.discriminator_config['discriminator blocks'], out_shape=self.config.data_config['image shape'], num_classes=self.config.data_config['num classes'], device=device, name="Conditional GAN")
        starting_epoch = 0
